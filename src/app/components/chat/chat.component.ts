...
import {DatePipe, formatDate, NgClass, NgOptimizedImage, NgStyle} from "@angular/common";
...
export class ChatComponent implements OnInit , OnChanges{
  ...
  subscribeToChat(){
    ...
    this.topicSubscription = this.stompService.watch(this.subscribingDestination).subscribe((message) => {
  ...
  messageName(message:MessageDTO){
    if(message.sender === this.userData.user.username){
      return this.userData.details.person.name + ' ' + this.userData.details.person.lastname;
    }else{
      return this.receiverUser.name + ' ' + this.receiverUser.lastname;
    }
  }
  onMessageSeen(event: any, message: MessageDTO){
    if(!message.seen && event === "VISIBLE" && message.sender !== this.userData.user.username){
      message.seen = true;
