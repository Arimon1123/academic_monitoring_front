...
export class AttendanceService {
  ...
  getAttendanceByAssignationId(assignationId: number) {
    return this.http.get<ResponseDTO<AttendanceDTO[]>>(
      `${this.API_URL}/attendance/assignation/${assignationId}`,
      { withCredentials: true },
    );
  }
  saveAllAttendance(attendance: AttendanceDTO[]) {
    return this.http.post<ResponseDTO<string>>(
      `${this.API_URL}/attendance`,
      attendance,
      { withCredentials: true },
    );
  }
  getAttendanceDatesByAssignationId(assignationId: number) {
    return this.http.get<ResponseDTO<AttendanceDateDTO[]>>(
      `${this.API_URL}/attendance/assignation/${assignationId}/date`,
      { withCredentials: true },
    );
  }
}
