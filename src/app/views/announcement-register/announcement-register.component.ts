...
export class AnnouncementRegisterComponent {
  ...
  saveAnnouncement() {
    ...
    const primaryIds = this.gradeList.filter(v =>{ return v.section === "Primaria"}).map( v=> v.id)
    const highSchoolIds = this.gradeList.filter(v =>{ return v.section === "Secundaria"}).map(v => v.id)
    console.log(primaryIds);
    let gradesIds: any[] = [];
     gradesIds = [...this.announcementForm.get('primarySection')?.value ? primaryIds : [] ]
     gradesIds = [...gradesIds, ...this.announcementForm.get('highSchoolSection')?.value ? highSchoolIds : [] ]
    announcement = {
       id: 0,
      title : this.announcementForm.get('title')?.value,
      message: this.announcementForm.get('message')?.value,
      date: new Date(),
      publishingDate: new Date(this.announcementForm.get(`publishingDate`)?.value),
